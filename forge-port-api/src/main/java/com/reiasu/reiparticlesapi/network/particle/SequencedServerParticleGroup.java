...
/**
 * A {@link ServerParticleGroup} that manages sequenced (indexed) particles,
 * allowing individual particles to be toggled on/off by index.
 */
public abstract class SequencedServerParticleGroup extends ServerParticleGroup {
    ...
    public Map.Entry<String, ParticleControlerDataBuffer<Integer>> toggleArgLeastIndex() {
        return Map.entry("least_index",
                ParticleControlerDataBuffers.INSTANCE.intValue(serverSequencedParticleCount));
    }

    public Map.Entry<String, ParticleControlerDataBuffer<long[]>> toggleArgStatus() {
        return Map.entry("index_status",
                ParticleControlerDataBuffers.INSTANCE.longArray(clientIndexStatus.clone()));
    }
