...
public final class ClientParticleStylePacketHandler {
    ...
    private static void handleChange(UUID uuid, Map<String, ParticleControlerDataBuffer<?>> args) {
    ...
    private static void handleCreate(UUID uuid, Map<String, ParticleControlerDataBuffer<?>> args) {
        String styleClassName = readString(args, "style_type");
        if (styleClassName == null) {
            return;
        }
        try {
            Class<?> rawClass = Class.forName(styleClassName);
            if (!ParticleGroupStyle.class.isAssignableFrom(rawClass)) {
                return;
            }
            Class<? extends ParticleGroupStyle> styleClass = (Class<? extends ParticleGroupStyle>) rawClass;
            ParticleStyleProvider<? extends ParticleGroupStyle> provider = ParticleStyleManager.getBuilder(styleClass);
            if (provider == null) {
                return;
            }
    ...
    private static void applyCommonArgs(ParticleGroupStyle style, Map<String, ? extends ParticleControlerDataBuffer<?>> args) {
    ...
    private static String readString(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        Object value = read(args, key);
        return value instanceof String s ? s : null;
    }

    private static Vec3 readVec3(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        Object value = read(args, key);
        return value instanceof Vec3 vec3 ? vec3 : null;
    }

    private static RelativeLocation readRelative(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        Object value = read(args, key);
    ...
    private static Double readDouble(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        Object value = read(args, key);
        return value instanceof Number number ? number.doubleValue() : null;
    }

    private static Long readLong(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        Object value = read(args, key);
        return value instanceof Number number ? number.longValue() : null;
    }

    private static Boolean readBoolean(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        Object value = read(args, key);
        return value instanceof Boolean b ? b : null;
    }

    private static Object read(Map<String, ? extends ParticleControlerDataBuffer<?>> args, String key) {
        ParticleControlerDataBuffer<?> buffer = args.get(key);
