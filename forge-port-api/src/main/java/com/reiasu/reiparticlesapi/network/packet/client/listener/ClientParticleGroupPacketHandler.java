...
import com.reiasu.reiparticlesapi.particles.control.group.ControlableParticleGroupProvider;
...
public final class ClientParticleGroupPacketHandler {
    ...
    private static void handleCreate(UUID groupUUID, Map<String, ParticleControlerDataBuffer<?>> args) {
        ...
        try {
            ...
            if (!ControlableParticleGroup.class.isAssignableFrom(rawClass)) return;
            Class<? extends ControlableParticleGroup> groupClass = (Class<? extends ControlableParticleGroup>) rawClass;

            ControlableParticleGroupProvider provider = ClientParticleGroupManager.INSTANCE.getBuilder(groupClass);
            if (provider == null) return;

            ControlableParticleGroup group = provider.createGroup(groupUUID, args);
    ...
    private static void handleChange(UUID groupUUID, Map<String, ParticleControlerDataBuffer<?>> args) {
        ControlableParticleGroup targetGroup = ClientParticleGroupManager.INSTANCE.getControlGroup(groupUUID);
        ...
        ControlableParticleGroupProvider builder = ClientParticleGroupManager.INSTANCE.getBuilder(targetGroup.getClass());
        if (builder != null) {
    ...
    private static void handleSequencedGroupArgs(SequencedParticleGroup group, Map<String, ParticleControlerDataBuffer<?>> args) {
    ...
    private static Object readValue(Map<String, ParticleControlerDataBuffer<?>> args, String key) {
        ParticleControlerDataBuffer<?> buf = args.get(key);
