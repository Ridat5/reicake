...
import com.reiasu.reiparticlesapi.particles.impl.ControlableEndRodEffect;
...
public abstract class SequencedParticleStyle extends ParticleGroupStyle {
    ...
    public abstract void readPacketArgsSequenced(Map<String, ? extends ParticleControlerDataBuffer<?>> args);
    ...
    public void addSingle() {
        ...
        Map<String, ParticleControlerDataBuffer<?>> args = buildChangeSingleStatusArgs(idx, true);
        change(g -> {}, args);
    }
    ...
    public void addMultiple(int count) {
        ...
        Map<String, ParticleControlerDataBuffer<?>> args = buildChangeMultipleStatusArgs(indices, true);
    ...
    public void removeSingle() {
        ...
        Map<String, ParticleControlerDataBuffer<?>> args = buildChangeSingleStatusArgs(idx, false);
        change(g -> {}, args);
    }
    ...
    public void removeMultiple(int count) {
        ...
        Map<String, ParticleControlerDataBuffer<?>> args = buildChangeMultipleStatusArgs(indices, false);
        change(g -> {}, args);
    }
    ...
    protected void toggleFromStatus(int index, boolean status) {
        ...
        if (status) {
            ...
            // Remove particle
            Controlable<?> particle = getParticles().get(data.getUuid());
            if (particle != null) {
                particle.remove();
    ...
    private void createWithIndex(SortedStyleData data, RelativeLocation rl) {
        ...
        if (displayer instanceof ParticleDisplayer.SingleParticleDisplayer) {
            ParticleControler controler = ControlParticleManager.INSTANCE.createControl(particleUUID);
            controler.setInitInvoker(data.getParticleHandler());
        }

        Vec3 toPos = new Vec3(getPos().x + rl.getX(), getPos().y + rl.getY(), getPos().z + rl.getZ());
        Controlable<?> controlable = displayer.display(toPos, (ClientLevel) getWorld());
        if (controlable == null) return;

        if (controlable instanceof ParticleControler) {
            data.getParticleControlerHandler().accept((ParticleControler) controlable);
        }
        getParticles().put(particleUUID, controlable);
        getParticleLocations().put(controlable, rl);
    }

    // ---- Packet args helpers ----

    private Map<String, ParticleControlerDataBuffer<?>> buildChangeSingleStatusArgs(int index, boolean status) {
        Map<String, ParticleControlerDataBuffer<?>> args = new HashMap<>();
        args.put("seq_index", ParticleControlerDataBuffers.INSTANCE.intValue(index));
        args.put("seq_status", ParticleControlerDataBuffers.INSTANCE.bool(status));
        args.put("seq_count", ParticleControlerDataBuffers.INSTANCE.intValue(displayedParticleCount));
        return args;
    }

    private Map<String, ParticleControlerDataBuffer<?>> buildChangeMultipleStatusArgs(int[] indices, boolean status) {
        Map<String, ParticleControlerDataBuffer<?>> args = new HashMap<>();
        args.put("seq_indices", ParticleControlerDataBuffers.INSTANCE.intArray(indices));
        args.put("seq_status", ParticleControlerDataBuffers.INSTANCE.bool(status));
        args.put("seq_count", ParticleControlerDataBuffers.INSTANCE.intValue(displayedParticleCount));
        return args;
    }
    ...
    public Map<String, ParticleControlerDataBuffer<?>> writePacketArgs() {
        Map<String, ParticleControlerDataBuffer<?>> args = new HashMap<>();
        args.put("seq_displayed_status", ParticleControlerDataBuffers.INSTANCE.longArray(getDisplayedStatus().clone()));
        args.put("seq_displayed_count", ParticleControlerDataBuffers.INSTANCE.intValue(displayedParticleCount));
        args.put("seq_current_index", ParticleControlerDataBuffers.INSTANCE.intValue(particleLinkageDisplayCurrentIndex));
        args.putAll(writePacketArgsSequenced());
        return args;
    }

    @Override
    public void readPacketArgs(Map<String, ? extends ParticleControlerDataBuffer<?>> args) {
        ParticleControlerDataBuffer<?> statusBuf = args.get("seq_displayed_status");
        if (statusBuf != null) {
            displayedStatus = (long[]) statusBuf.getLoadedValue();
        }
        ParticleControlerDataBuffer<?> countBuf = args.get("seq_displayed_count");
        if (countBuf != null) {
            displayedParticleCount = (int) countBuf.getLoadedValue();
        }
        ParticleControlerDataBuffer<?> indexBuf = args.get("seq_current_index");
        if (indexBuf != null) {
    ...
    public static final class SortedStyleDataBuilder {
        private Function<UUID, ParticleDisplayer> displayerBuilder =
                uuid -> ParticleDisplayer.Companion.withSingle(new ControlableEndRodEffect(uuid, false));
        private final List<Consumer<ControlableParticle>> particleHandlers = new ArrayList<>();
        private final List<Consumer<ParticleControler>> particleControlerHandlers = new ArrayList<>();

        public SortedStyleDataBuilder addParticleHandler(Consumer<ControlableParticle> builder) {
            particleHandlers.add(builder);
            return this;
        }

        public SortedStyleDataBuilder addParticleControlerHandler(Consumer<ParticleControler> builder) {
        ...
        public SortedStyleDataBuilder clearParticleControlers() {
        ...
        public SortedStyleDataBuilder removeParticleControler(int index) {
            if (index >= 0 && index < particleControlerHandlers.size()) {
                particleControlerHandlers.remove(index);
            }
            return this;
        }
        ...
        public SortedStyleData build(int order) {
            ...
            data.withParticleHandler(p -> {
                for (Consumer<ControlableParticle> handler : particleHandlers) {
                    handler.accept(p);
                }
            });
            data.withParticleControlerHandler(c -> {
                for (Consumer<ParticleControler> handler : particleControlerHandlers) {
                    handler.accept(c);
                }
            });
            return data;
