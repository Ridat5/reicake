...
public record PacketParticleGroupS2C(
    ...
    public static void encode(PacketParticleGroupS2C packet, FriendlyByteBuf buf) {
        ...
        for (Map.Entry<String, ParticleControlerDataBuffer<?>> entry : packet.args.entrySet()) {
            byte[] encoded = ParticleControlerDataBuffers.INSTANCE.encode(entry.getValue());
    ...
    public static PacketParticleGroupS2C decode(FriendlyByteBuf buf) {
        ...
        while (buf.readableBytes() > 0) {
            ...
            buf.readBytes(raw);
            args.put(key, ParticleControlerDataBuffers.INSTANCE.decodeToBuffer(raw));
        }
