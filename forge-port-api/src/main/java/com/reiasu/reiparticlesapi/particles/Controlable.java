...
public interface Controlable<T> {
    ...
    default void load(Map<String, ? extends ParticleControlerDataBuffer<?>> args) {
    }

    default Map<String, ParticleControlerDataBuffer<?>> toArgs() {
        return Map.of();
    }

    default void change(Map<String, ? extends ParticleControlerDataBuffer<?>> args) {
        load(args);
    }
}
