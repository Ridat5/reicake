...
public class ControlableFireworkEffect implements ControlableParticleEffect {
    private UUID uuid;
    private final boolean faceToPlayer;

    public ControlableFireworkEffect(UUID uuid, boolean faceToPlayer) {
        this.uuid = uuid;
        this.faceToPlayer = faceToPlayer;
    }

    public ControlableFireworkEffect(UUID uuid, boolean faceToPlayer, int ignored, Object ignored2) {
    ...
    @Override
    public ControlableFireworkEffect clone() {
        return new ControlableFireworkEffect(UUID.randomUUID(), faceToPlayer);
    }

    @Override public boolean getFaceToPlayer() { return faceToPlayer; }

    @Override public ParticleType<?> getType() { return ReiModParticles.CONTROLABLE_FIREWORK.get(); }
    @Override public void writeToNetwork(FriendlyByteBuf buf) {
        buf.writeUUID(uuid); buf.writeBoolean(faceToPlayer);
    }
    @Override public String writeToString() { return "coo:firework"; }
}
