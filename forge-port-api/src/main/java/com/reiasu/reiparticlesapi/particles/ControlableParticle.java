...
/**
 * Core client-side particle class that can be remotely controlled via a {@link ParticleControler}.
 * <p>
 * Extends {@link TextureSheetParticle} (Forge 1.20.1) and provides:
 * <ul>
 *   <li>UUID-based control via {@link ControlParticleManager}</li>
 ...
 * </ul>
 */
public abstract class ControlableParticle extends TextureSheetParticle {
    ...
    private final ParticleControler controler;
    ...
    public ControlableParticle(ClientLevel world, Vec3 pos, Vec3 velocity, UUID controlUUID, boolean faceToCamera) {
        ...
        ParticleControler ctrl = ControlParticleManager.INSTANCE.getControl(this.controlUUID);
        if (ctrl == null) {
            throw new IllegalStateException("No ParticleControler registered for UUID " + this.controlUUID);
        }
        this.controler = ctrl;
        this.light = 15;
        ...
        this.controler.loadParticle(this);
        this.controler.particleInit();
        this.lastRotate = new Vector3f(this.previewPitch, this.previewYaw, this.oRoll);
    }

    public ControlableParticle(ClientLevel world, Vec3 pos, Vec3 velocity, UUID controlUUID) {
    ...
    public ParticleControler getControler() {
        return controler;
    }
    ...
    public ControlableParticle setInterpolator(ParticleLerpInterpolator newInterpolator) {
    ...
    /**
     * Renders a single quad facing the given orientation.
     */
    ...
    /**
     * Renders a double-sided quad (front + back face) for non-camera-facing particles.
     */
    ...
    /**
     * Emit a single vertex for the particle quad.
     * Uses Forge 1.20.1 VertexConsumer API with endVertex().
     */
