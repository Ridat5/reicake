...
import com.reiasu.reiparticlesapi.particles.control.group.ControlableParticleGroup;
 ...
 * a {@link Controlable} handle that can be used for subsequent control operations.
 */
public interface ParticleDisplayer {
     ...
     *
     * @return a {@link Controlable} handle, or null if spawning failed
     */
    @Nullable
    ...
    static ParticleDisplayer withSingle(ControlableParticleEffect effect) {
        return new SingleParticleDisplayer(effect);
    }

    static ParticleDisplayer withGroup(ControlableParticleGroup group) {
    ...
    class Companion {
        public ParticleDisplayer withSingle(ControlableParticleEffect effect) {
            return ParticleDisplayer.withSingle(effect);
        }

        public ParticleDisplayer withGroup(ControlableParticleGroup group) {
    ...
    /**
     * Displays a single ControlableParticle via its ControlableParticleEffect.
     */
    class SingleParticleDisplayer implements ParticleDisplayer {
        private final ControlableParticleEffect effect;

        public SingleParticleDisplayer(ControlableParticleEffect effect) {
            this.effect = effect;
        }

        public ControlableParticleEffect getEffect() {
            return effect;
        }

        @Override
        @Nullable
        public Controlable<?> display(Vec3 loc, ClientLevel world) {
            // Spawn the particle effect in the world (ControlableParticleEffect extends ParticleOptions)
            world.addAlwaysVisibleParticle(effect, true, loc.x, loc.y, loc.z, 0.0, 0.0, 0.0);
            ParticleControler controler = ControlParticleManager.INSTANCE.getControl(effect.getControlUUID());
            return controler;
        }
    }

    /**
     * Displays a ControlableParticleGroup.
     */
    class ParticleGroupDisplayer implements ParticleDisplayer {
        private final ControlableParticleGroup group;

        public ParticleGroupDisplayer(ControlableParticleGroup group) {
            this.group = group;
        }

        public ControlableParticleGroup getGroup() {
            return group;
        }
    ...
    class DisplayEntityDisplayer implements ParticleDisplayer {
        ...
        public Controlable<?> display(Vec3 loc, ClientLevel world) {
            ...
            return null; // DisplayEntity does not implement Controlable
        }
    }
