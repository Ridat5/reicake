...
public abstract class AbstractBarrage implements Barrage {
    ...
    protected AbstractBarrage(Vec3 loc, ServerLevel world, HitBox hitBox,
                              ServerControler<?> bindControl, BarrageOption options) {
        this.loc = loc;
        this.world = world;
        this.hitBox = hitBox;
    ...
    @Override
    public ServerControler<?> getBindControl() { return bindControl; }
     ...
     */
    public Vec3 getControlerLocation() {
        return getLoc();
    }
    ...
    public void tick() {
        ...
        if (options.isEnableSpeed()) {
            Vec3 move = direction.normalize().scale(options.getSpeed());
            loc = loc.add(move);
        ...
        // Teleport bound controller
        // Teleport bound controller to follow barrage position
     ...
     */
    public void remove() {
        // Note: ServerControler.remove() not in current interface; call cancel()
        bindControl.cancel();
        isValid = false;
    }
